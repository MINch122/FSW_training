...
int CFE_SRL_RtableCSP(csp_iface_t *Iface) {
	...
	Status = csp_rtable_set(CSP_NODE_LTRX, CSP_ID_HOST_SIZE, Iface, CSP_NO_VIA_ADDRESS);
	if (Status != CSP_ERR_NONE) return CFE_SRL_CSP_RTABLE_SET_ERR;

	Status = csp_rtable_set(CSP_NODE_PAY_IFB, CSP_ID_HOST_SIZE, Iface, CSP_NO_VIA_ADDRESS);
	if (Status != CSP_ERR_NONE) return CFE_SRL_CSP_RTABLE_SET_ERR;

	Status = csp_rtable_set(CSP_NODE_PAY_EXP, CSP_ID_HOST_SIZE, Iface, CSP_NO_VIA_ADDRESS);
	if (Status != CSP_ERR_NONE) return CFE_SRL_CSP_RTABLE_SET_ERR;

	Status = csp_rtable_set(CSP_NODE_AIOBC, CSP_ID_HOST_SIZE, Iface, CSP_NO_VIA_ADDRESS);
	if (Status != CSP_ERR_NONE) return CFE_SRL_CSP_RTABLE_SET_ERR;
...
int CFE_SRL_AllNodeConfigCSP(void) {
	...
	CFE_SRL_NodeConfigCSP(CSP_NODE_LTRX, CSP_PRIO_NORM, CSP_TIMEOUT(1), CSP_O_CRC32);
	CFE_SRL_NodeConfigCSP(CSP_NODE_PAY_IFB, CSP_PRIO_NORM, CSP_TIMEOUT(1), CSP_O_CRC32);
	CFE_SRL_NodeConfigCSP(CSP_NODE_PAY_EXP, CSP_PRIO_NORM, CSP_TIMEOUT(1), CSP_O_CRC32);
	CFE_SRL_NodeConfigCSP(CSP_NODE_AIOBC, CSP_PRIO_NORM, CSP_TIMEOUT(1), CSP_O_CRC32);
