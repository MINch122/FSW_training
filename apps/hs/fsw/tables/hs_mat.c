...
HS_MatTableEntry_t HS_MsgActs_Tbl[HS_MAX_MSG_ACT_TYPES] = {
     ...
     * Lab bring-up: keep the command definition, but do not let HS issue it automatically.
     */
    ...
    /*   3 - EPS PMU reboot
     * Lab bring-up: disable autonomous EPS reboot commands.
     */
    {.EnableState = HS_MAT_STATE_DISABLED,
     .Cooldown    = 10,
     .HsMsg.cmd4  = {
         .CommandHeader = CFE_MSG_CMD_HDR_INIT(EPS_CMD_MID, HS_MEMBER_SIZE(cmd4), EPS_CSP_REBOOT_CC, 0xDE),
         .Payload       = {.csp_node = EPS_P80_PMU_CSP_NODE}}},
    
    /*   4  - TO child restart
     * Lab bring-up: keep manual recovery available, but disable autonomous restart.
     */
    {.EnableState = HS_MAT_STATE_DISABLED,
     .Cooldown    = 1,
