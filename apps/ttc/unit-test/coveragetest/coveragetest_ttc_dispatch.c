...
void Test_TTC_TaskPipe(void)
{
    ...
    UT_CHECKEVENT_SETUP(&EventTest, TTC_MID_ERR_EID, "TTC: invalid command packet,MID = 0x%x");
...
void Test_TTC_ProcessGroundCommand(void)
{
    ...
    UT_CHECKEVENT_SETUP(&EventTest, TTC_CMD_LEN_ERR_EID,
    ...
    UtAssert_STUB_COUNT(TTC_NoopCmd, 1);
    ...
    UtAssert_STUB_COUNT(TTC_ResetCountersCmd, 1);
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CHECKEVENT_SETUP(&EventTest, TTC_CC_ERR_EID, "Invalid ground command code: CC = %d");
    TTC_ProcessGroundCommand(&TestMsg.SBBuf);
