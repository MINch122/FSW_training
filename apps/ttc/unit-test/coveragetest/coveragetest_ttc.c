...
**
** Purpose:
** Coverage Unit Test cases for the Ttclication
**
** Notes:
...
void Test_TTC_Main(void)
{
    ...
    /*
     * TTC_Main does not return a value,
     ...
     * The first is for TTC_Init().  As this is in the same
     ...
     * the TTC_Init() failure path was taken.
     */
    TTC_Main();
     ...
     */
    UtAssert_UINT32_EQ(TTC_AppData.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, TTC_PIPE_ERR_EID, "TTC: SB Pipe Read Error, App Will Exit");
...
void Test_TTC_Init(void)
{
    ...
    UtAssert_INT32_EQ(TTC_Init(), CFE_SUCCESS);
     ...
     */
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(TTC_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(TTC_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    /* Third subscription (1Hz wakeup) fails */
