...
static int32 PAY_SLT_FetchParamCompat(PAY_SLT_Params_t *Payload)
{
    ...
    switch (Payload->type) {
        case GS_PARAM_UINT8:
            element_size = sizeof(Payload->param.u8[0]);
            capacity = sizeof(Payload->param.u8) / element_size;
            break;
        case GS_PARAM_UINT16:
            element_size = sizeof(Payload->param.u16[0]);
            capacity = sizeof(Payload->param.u16) / element_size;
            break;
        case GS_PARAM_UINT32:
            element_size = sizeof(Payload->param.u32[0]);
            capacity = sizeof(Payload->param.u32) / element_size;
            break;
        case GS_PARAM_INT8:
            element_size = sizeof(Payload->param.i8[0]);
            capacity = sizeof(Payload->param.i8) / element_size;
            break;
        case GS_PARAM_INT16:
            element_size = sizeof(Payload->param.i16[0]);
            capacity = sizeof(Payload->param.i16) / element_size;
            break;
        case GS_PARAM_STRING:
            element_size = sizeof(Payload->param.str[0]);
            capacity = sizeof(Payload->param.str);
            break;
        case GS_PARAM_FLOAT:
            element_size = sizeof(Payload->param.flt[0]);
            capacity = sizeof(Payload->param.flt) / element_size;
            break;
        default:
            return GS_ERROR_ARG;
    }
...

    {
        ...
        for (size_t i = 0; i < actual_count; ++i) {
            ...
            uint8 *destination = ((uint8 *)&Payload->param) + (i * element_size);
            const uint8 *source = &reply[item_offset + PAY_SLT_RPARAM_ADDR_SIZE];
            ...
            switch (Payload->type) {
                ...
                case GS_PARAM_INT8:
                    *destination = *source;
                    break;
                case GS_PARAM_UINT16: {
                    uint16 value = PAY_SLT_LoadBe16(source);
                    memcpy(destination, &value, sizeof(value));
                    break;
                }
                case GS_PARAM_INT16: {
                    int16 value = (int16)PAY_SLT_LoadBe16(source);
                    memcpy(destination, &value, sizeof(value));
                    break;
                }
                case GS_PARAM_UINT32:
                case GS_PARAM_FLOAT: {
                    uint32 value = PAY_SLT_LoadBe32(source);
                    memcpy(destination, &value, sizeof(value));
                    break;
                }
                default:
...
// RParam 호환 SET 트랜잭션으로 파라미터 값 전송
...
int32 PAY_SLT_SetParam(uint8 node, uint8 table, uint16 addr, uint8 type, void *value) {
    ...
    Status = PAY_SLT_SetParamCompat(node, table, addr, type, value);
...
// RS422 바이트 단위로 수신 (초기 전송 시간 간격 적용)
...
/*                                                                            */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * **/
// 파라미터 타입을 문자 레이블로 변환 (U8, I16, FLT 등)
