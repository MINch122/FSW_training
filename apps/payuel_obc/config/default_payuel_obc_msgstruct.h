...
/**
 * 0x21 - 모터 모드 제어
 * TX: [0x21][L_Speed(int8)][R_Speed(int8)][Duration(uint8)][Instance(uint8)][CRC16(BE)] = 7 bytes
 */
typedef struct
{
    ...
    uint8  Instance;  /**< 인스턴스, 0~255 */
} PAYUEL_OBC_MotorModeCmd_t;

/**
 * 0x41 - 사진 촬영 요청
 * TX: [0x41][ImageSlot][CameraNumber][CRC16(BE)] = 5 bytes
 */
typedef struct
{
    CFE_MSG_CommandHeader_t CommandHeader;
    uint8  ImageSlot;    /**< 저장될 이미지 슬롯 번호, 0~255 */
    uint8  CameraNumber; /**< 촬영에 사용할 카메라 번호, 0~1  */
} PAYUEL_OBC_CamShotCmd_t;

/**
 * 0x44 - 이미지 다운로드 메타 요청
 * TX: [0x44][ImageSlot][ImageNumber][CRC16(BE)] = 5 bytes
 */
typedef struct
{
    CFE_MSG_CommandHeader_t CommandHeader;
    uint8 ImageSlot;   /**< 다운로드할 이미지 슬롯 번호, 0~255 */
    uint8 ImageNumber; /**< 슬롯 내 이미지 번호, 0~9 */
...
typedef struct
{
    ...
    uint8 DataNumber; /**< 슬롯 내 데이터 번호, 0~9 */
...
typedef struct
{
    ...
    uint16_t BootCount;       /**< 부팅 횟수, 0~65535 */
    uint32_t BootTime;        /**< 부팅 시간 (초), 0~4294967295 */
    uint16_t ShutterCount;    /**< 카메라 촬영 횟수, 0~65535 */
