        ...
        if (RtTblEntry->OutputPktAddr != NULL)
        {
            ...
            if (CFE_SB_MsgId_Equal(ThisEntrysOutMid, WhichMidToSend))
            {
...
#endif
                {
                    ...
                    OS_printf("Header: MID 0x%02X%02X || Length 0x%02X%02X ", OutBuffer->Msg.CCSDS.Pri.StreamId[0], OutBuffer->Msg.CCSDS.Pri.StreamId[1], OutBuffer->Msg.CCSDS.Pri.Length[0], OutBuffer->Msg.CCSDS.Pri.Length[1]);
