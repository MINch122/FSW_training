                     ...
                     uint8 retType)
{
    ...
    MEOW_AppData.Report.Payload.ReturnDataSize = dataSize;
    if (data && dataSize)
        memcpy(MEOW_AppData.Report.Payload.ReturnValue,
               data,
               dataSize > MEOW_MISSION_MAX_READ_LEN ? MEOW_MISSION_MAX_READ_LEN : dataSize);
...
CFE_Status_t MEOW_SendHkCmd(const MEOW_SendHkCmd_t* msg)
{
    ...
    CFE_SB_TimeStampMsg(CFE_MSG_PTR(MEOW_AppData.HkTlm.TelemetryHeader));
    CFE_SB_TransmitMsg(CFE_MSG_PTR(MEOW_AppData.HkTlm.TelemetryHeader), true);
    return CFE_SUCCESS;
}
