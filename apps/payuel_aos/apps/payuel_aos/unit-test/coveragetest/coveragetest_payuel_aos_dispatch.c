...
void Test_PAYUEL_AOS_TaskPipe(void)
{
    /*
     * Test Case For:
     * void PAYUEL_AOS_TaskPipe
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        PAYUEL_AOS_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_AOS_MID_ERR_EID, "PAYUEL_AOS: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(PAYUEL_AOS_CMD_MID);
    FcnCode   = PAYUEL_AOS_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    PAYUEL_AOS_TaskPipe(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(PAYUEL_AOS_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    PAYUEL_AOS_TaskPipe(&TestMsg.SBBuf);
...
void Test_PAYUEL_AOS_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void PAYUEL_AOS_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        PAYUEL_AOS_NoopCmd_t          Noop;
        PAYUEL_AOS_ResetCountersCmd_t Reset;
        PAYUEL_AOS_ProcessCmd_t       Process;
        PAYUEL_AOS_DisplayParamCmd_t  DisplayParam;
    } TestMsg;
    UT_CheckEvent_t EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_AOS_CMD_LEN_ERR_EID,
    ...
    PAYUEL_AOS_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_STUB_COUNT(PAYUEL_AOS_NoopCmd, 1);
    ...
    FcnCode = PAYUEL_AOS_RESET_COUNTERS_CC;
    ...
    UtAssert_STUB_COUNT(PAYUEL_AOS_ResetCountersCmd, 1);
    ...
    /* note this will end up calling PAYUEL_AOS_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = PAYUEL_AOS_PROCESS_CC;
    ...
    UtAssert_STUB_COUNT(PAYUEL_AOS_ProcessCmd, 1);
    ...
    FcnCode = PAYUEL_AOS_DISPLAY_PARAM_CC;
    ...
    UtAssert_STUB_COUNT(PAYUEL_AOS_DisplayParamCmd, 1);
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_AOS_CC_ERR_EID, "Invalid ground command code: CC = %d");
    PAYUEL_AOS_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_PAYUEL_AOS_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool PAYUEL_AOS_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    ...
    PAYUEL_AOS_VerifyCmdLength(NULL, size);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    PAYUEL_AOS_VerifyCmdLength(NULL, size + 1);
...
{
    ADD_TEST(PAYUEL_AOS_TaskPipe);
    ADD_TEST(PAYUEL_AOS_ProcessGroundCommand);
    ADD_TEST(PAYUEL_AOS_VerifyCmdLength);
}
