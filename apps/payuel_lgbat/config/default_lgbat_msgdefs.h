...
// BMS I2C Data Structures (one per Data ID, all packed)
...
// Data ID 0x09: BMS Status (bitfields)
typedef struct {
    uint8_t  ID;              
    uint8_t  BMS_Wakeup;      
...
// 0x09 bit-field extraction macros
...
// Data ID 0x0A: Failure Status (bitfields)
typedef struct {
    uint8_t  ID;           
    uint8_t  FailStatus2;  
    uint8_t  FailStatus3; 
...
// 0x0A bit-field extraction macros
...
// True if any fault bit in FailStatus2 or FailStatus3 is set
#define LGBAT_0x0A_ANY_FAIL2(b)              ((b) != 0x00)
#define LGBAT_0x0A_ANY_FAIL3(b)              (((b) & 0xA0) != 0x00)  // bit7 or bit5
...
// Data ID 0x0C: BMS PCB Current and Boost Status
typedef struct {
    uint8_t  ID;                   
    int16_t  BMS_Current;          
...
// Aggregate structure holding all 12 Data IDs
typedef struct {
...
typedef struct {
    ...
    // Key BMS values for quick health check
...
typedef struct {
    ...
    uint8_t  CmdCounter;
    uint8_t  CmdErrCounter;
    uint8_t  PowerApplied;     // 1 = 3.3V on
    uint8_t  Spare;
    ...
    // Data ID 0x07: Cell Temperatures (ICD byte order: T02 first, T01 second)
    ...
    uint8_t  BMS_Wakeup;          // HW wakeup state from ICD byte1 (read-only from BMS)
    uint8_t  BMS_Wakeup_Signal;   // Extracted from WakeupHoldStatus bit1
    uint8_t  Failure_Level;
    uint8_t  DFET_Status;
    uint8_t  CFET_Status;
