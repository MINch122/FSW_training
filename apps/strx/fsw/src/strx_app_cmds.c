...
{
    STRX_APP_Data.RptPkt.Report.CommandCode = cc;

    if (status == DEVICE_SUCCESS) {
        STRX_APP_Data.RptPkt.Report.ReturnType = RPT_RETTYPE_SUCCESS;
        STRX_APP_Data.RptPkt.Report.ReturnCode = DEVICE_SUCCESS;
    } else {
        STRX_APP_Data.RptPkt.Report.ReturnType = RPT_RETTYPE_HW;
        STRX_APP_Data.RptPkt.Report.ReturnCode = status;
    }
}
...
static inline void STRX_RptCopy(const void *src, size_t len)
{
    ...
    size_t maxbuf = sizeof(STRX_APP_Data.RptPkt.Report.ReturnValue);
    if (n > maxbuf) n = maxbuf;

    memcpy(STRX_APP_Data.RptPkt.Report.ReturnValue, src, n);
    STRX_APP_Data.RptPkt.Report.ReturnDataSize = (uint16_t)n;
}
...
void STRX_ReportHousekeeping(void)
{
    ...
    if (STRX_RXCONF_GetGuard(&hk->rx_guard) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GetLastRssi(&hk->LastRssi) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GetBootCount(&hk->BootCount) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GetBootCause(&hk->BootCause) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GetTotTxBytes(&hk->TotTxBytes) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GetTotRxBytes(&hk->TotRxBytes) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GET_HWDET(&hk->hw_det) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GET_RXMODE(&hk->rxmode) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GET_GND_WDT_CNT(&hk->gnd_wdt_cnt) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GET_GND_WDT_LEFT(&hk->gnd_wdt_left) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TXCONF_GetFreq(&hk->tx_freq) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TXCONF_GetBaud(&hk->tx_baud) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TXCONF_GetGuard(&hk->tx_guard) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
    if (STRX_TLM_GetRssibusy(&hk->rssibusy) != DEVICE_SUCCESS) STRX_APP_Data.AppCnt.DeviceErrCounter++;
...
CFE_Status_t STRX_APP_NoopCmd(const STRX_APP_NoopCmd_t *Msg)
{
    ...
    CFE_EVS_SendEvent(STRX_APP_NOOP_INF_EID, CFE_EVS_EventType_INFORMATION, "STRX: NOOP command");
...
CFE_Status_t STRX_APP_ResetCountersCmd(const STRX_APP_ResetCountersCmd_t *Msg)
{
    ...
    STRX_APP_Data.AppCnt.AppCmdCounter = 0;
    STRX_APP_Data.AppCnt.AppErrCounter = 0;
    STRX_APP_Data.AppCnt.DeviceErrCounter = 0;

    CFE_EVS_SendEvent(STRX_APP_RESET_INF_EID, CFE_EVS_EventType_INFORMATION, "STRX: RESET count command");
...
CFE_Status_t STRX_APP_ResetDeviceCmdCountersCmd(const STRX_APP_ResetDeviceCmdCountersCmd_t *Msg)
{
    ...
    STRX_APP_Data.AppCnt.DeviceErrCounter = 0;
    CFE_EVS_SendEvent(STRX_APP_RESET_DEVICE_COUNTER_EID, CFE_EVS_EventType_INFORMATION,
...
CFE_Status_t STRX_APP_ResetAppCmdCountersCmd(const STRX_APP_ResetAppCmdCountersCmd_t *Msg)
{
    ...
    CFE_EVS_SendEvent(STRX_APP_RESET_APP_COUNTER_EID, CFE_EVS_EventType_INFORMATION,
...
void STRX_GndwdtClearCmd(void){
    ...
    if (status != DEVICE_SUCCESS) {
        CFE_EVS_SendEvent(STRX_APP_CC_ERR_EID, CFE_EVS_EventType_ERROR,
