...
/*
** File: coveragetest_payuel_roma.c
**
** Purpose:
...
void Test_PAYUEL_ROMA_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void PAYUEL_ROMA_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    ...
    PAYUEL_ROMA_SendHkCmd(NULL);

    /* Confirm message sent*/
    UtAssert_STUB_COUNT(CFE_SB_TransmitMsg, 1);
    UtAssert_ADDRESS_EQ(MsgSend, &PAYUEL_ROMA_Data.HkTlm);

    /* Confirm timestamp msg address */
    UtAssert_STUB_COUNT(CFE_SB_TimeStampMsg, 1);
    UtAssert_ADDRESS_EQ(MsgTimestamp, &PAYUEL_ROMA_Data.HkTlm);
...
void Test_PAYUEL_ROMA_NoopCmd(void)
{
    /*
     * Test Case For:
     * void PAYUEL_ROMA_NoopCmd( const PAYUEL_ROMA_Noop_t *Msg )
     */
    PAYUEL_ROMA_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    /* test dispatch of NOOP */
    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_ROMA_NOOP_INF_EID, NULL);

    UtAssert_INT32_EQ(PAYUEL_ROMA_NoopCmd(&TestMsg), CFE_SUCCESS);
...
void Test_PAYUEL_ROMA_ResetCountersCmd(void)
{
    /*
     * Test Case For:
     * void PAYUEL_ROMA_ResetCounters( const PAYUEL_ROMA_ResetCounters_t *Msg )
     */
    PAYUEL_ROMA_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_ROMA_RESET_INF_EID, "Roma-SP: RESET command");

    UtAssert_INT32_EQ(PAYUEL_ROMA_ResetCountersCmd(&TestMsg), CFE_SUCCESS);
...
void Test_PAYUEL_ROMA_ProcessCmd(void)
{
    /*
     * Test Case For:
     * void  PAYUEL_ROMA_ProcessCmd( const PAYUEL_ROMA_ProcessCmd_t *Msg )
     */
    PAYUEL_ROMA_ProcessCmd_t   TestMsg;
    PAYUEL_ROMA_ExampleTable_t TestTblData;
    void *                    TblPtr = &TestTblData;

    memset(&TestTblData, 0, sizeof(TestTblData));
    memset(&TestMsg, 0, sizeof(TestMsg));

    /* Provide some table data for the PAYUEL_ROMA_Process() function to use */
    TestTblData.Int1 = 40;
    TestTblData.Int2 = 50;
    UT_SetDataBuffer(UT_KEY(CFE_TBL_GetAddress), &TblPtr, sizeof(TblPtr), false);
    UtAssert_INT32_EQ(PAYUEL_ROMA_ProcessCmd(&TestMsg), CFE_SUCCESS);
     ...
     * PAYUEL_ROMA_GetCrc() function is a stub.
     */
    ...
    UtAssert_INT32_EQ(PAYUEL_ROMA_ProcessCmd(&TestMsg), CFE_TBL_ERR_UNREGISTERED);
    ...
    UtAssert_INT32_EQ(PAYUEL_ROMA_ProcessCmd(&TestMsg), CFE_TBL_ERR_NO_ACCESS);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 4);
}

void Test_PAYUEL_ROMA_DisplayParamCmd(void)
{
    /*
     * Test Case For:
     * void  PAYUEL_ROMA_DisplayParamCmd( const PAYUEL_ROMA_DisplayParamCmd_t *Msg )
     */
    PAYUEL_ROMA_DisplayParamCmd_t TestMsg;
    UT_CheckEvent_t              EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_ROMA_VALUE_INF_EID, "PAYUEL_ROMA: ValU32=%lu, ValI16=%d, ValStr=%s");
    TestMsg.Payload.ValU32 = 10;
    TestMsg.Payload.ValI16 = -4;
    snprintf(TestMsg.Payload.ValStr, sizeof(TestMsg.Payload.ValStr), "Hello");

    UtAssert_INT32_EQ(PAYUEL_ROMA_DisplayParamCmd(&TestMsg), CFE_SUCCESS);
    /*
     * Confirm that the event was generated
     */
    UtAssert_UINT32_EQ(EventTest.MatchCount, 1);
...
{
    ADD_TEST(PAYUEL_ROMA_ReportHousekeeping);
    ADD_TEST(PAYUEL_ROMA_NoopCmd);
    ADD_TEST(PAYUEL_ROMA_ResetCountersCmd);
    ADD_TEST(PAYUEL_ROMA_ProcessCmd);
    ADD_TEST(PAYUEL_ROMA_DisplayParamCmd);
}
