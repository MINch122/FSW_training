...
void Test_PAYUEL_ROMA_TblValidationFunc(void)
{
    /*
     * Test Case For:
    ...
    UtAssert_INT32_EQ(PAYUEL_ROMA_TblValidationFunc(&TestTblData), CFE_SUCCESS);

    /* error case should return PAYUEL_ROMA_TABLE_OUT_OF_RANGE_ERR_CODE */
    TestTblData.Int1 = 1 + PAYUEL_ROMA_TBL_ELEMENT_1_MAX;
    UtAssert_INT32_EQ(PAYUEL_ROMA_TblValidationFunc(&TestTblData), PAYUEL_ROMA_TABLE_OUT_OF_RANGE_ERR_CODE);
}

void Test_PAYUEL_ROMA_GetCrc(void)
{
    /*
     * Test Case For:
...
{
    ADD_TEST(PAYUEL_ROMA_TblValidationFunc);
    ADD_TEST(PAYUEL_ROMA_GetCrc);
}
