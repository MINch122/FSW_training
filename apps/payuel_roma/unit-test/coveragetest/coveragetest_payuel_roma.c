...
void Test_PAYUEL_ROMA_Main(void)
{
    ...
    /*
     * PAYUEL_ROMA_Main does not return a value,
     ...
     * The first is for PAYUEL_ROMA_Init().  As this is in the same
     ...
     * the PAYUEL_ROMA_Init() failure path was taken.
     */
    PAYUEL_ROMA_Main();
     ...
     */
    UtAssert_UINT32_EQ(PAYUEL_ROMA_Data.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_ROMA_PIPE_ERR_EID, "UELYSYS Payload Roma-SP: SB Pipe Read Error, App Will Exit");
...
void Test_PAYUEL_ROMA_Init(void)
{
    /*
     * Test Case For:
    ...
    UtAssert_INT32_EQ(PAYUEL_ROMA_Init(), CFE_SUCCESS);
     ...
     */
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(PAYUEL_ROMA_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(PAYUEL_ROMA_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UtAssert_INT32_EQ(PAYUEL_ROMA_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
...
{
    ADD_TEST(PAYUEL_ROMA_Main);
    ADD_TEST(PAYUEL_ROMA_Init);
}
