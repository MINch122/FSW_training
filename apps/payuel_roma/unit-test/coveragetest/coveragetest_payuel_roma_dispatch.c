...
void Test_PAYUEL_ROMA_TaskPipe(void)
{
    ...
    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_ROMA_MID_ERR_EID, "Roma-SP: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(PAYUEL_ROMA_CMD_MID);
    FcnCode   = PAYUEL_ROMA_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    PAYUEL_ROMA_TaskPipe(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(PAYUEL_ROMA_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    PAYUEL_ROMA_TaskPipe(&TestMsg.SBBuf);
...
void Test_PAYUEL_ROMA_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void PAYUEL_ROMA_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        PAYUEL_ROMA_NoopCmd_t          Noop;
        PAYUEL_ROMA_ResetCountersCmd_t Reset;
        PAYUEL_ROMA_ProcessCmd_t       Process;
        PAYUEL_ROMA_DisplayParamCmd_t  DisplayParam;
    } TestMsg;
    UT_CheckEvent_t EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_ROMA_CMD_LEN_ERR_EID,
    ...
    PAYUEL_ROMA_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_STUB_COUNT(PAYUEL_ROMA_NoopCmd, 1);
    ...
    FcnCode = PAYUEL_ROMA_RESET_COUNTERS_CC;
    ...
    UtAssert_STUB_COUNT(PAYUEL_ROMA_ResetCountersCmd, 1);
    ...
    /* note this will end up calling PAYUEL_ROMA_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = PAYUEL_ROMA_PROCESS_CC;
    ...
    UtAssert_STUB_COUNT(PAYUEL_ROMA_ProcessCmd, 1);
    ...
    FcnCode = PAYUEL_ROMA_DISPLAY_PARAM_CC;
    ...
    UtAssert_STUB_COUNT(PAYUEL_ROMA_DisplayParamCmd, 1);
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CHECKEVENT_SETUP(&EventTest, PAYUEL_ROMA_CC_ERR_EID, "Invalid ground command code: CC = %d");
    PAYUEL_ROMA_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_PAYUEL_ROMA_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool PAYUEL_ROMA_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    ...
    PAYUEL_ROMA_VerifyCmdLength(NULL, size);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    PAYUEL_ROMA_VerifyCmdLength(NULL, size + 1);
...
void UtTest_Setup(void)
{
    ...
    ADD_TEST(PAYUEL_ROMA_ProcessGroundCommand);
    ADD_TEST(PAYUEL_ROMA_VerifyCmdLength);
}
