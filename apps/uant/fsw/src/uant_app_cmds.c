...
{   
    ...
    */
    ...
    status=ISIS_UANT_ReportDeploymentStatus(&UANT_APP_Data.HkTlm.Payload.deploystatus); 
    ...
    for (uint8_t ant = 1; ant <= 4; ant++)
    {
        ...
        {
            case 1:
                status=ISIS_UANT_ReportAntennaActivationCount(1, &UANT_APP_Data.HkTlm.Payload.ant1actvcnt);
                if (status != CFE_SUCCESS)
                {
                    CFE_EVS_SendEvent(UANT_APP_GET_STATUS_ERR_EID, CFE_EVS_EventType_ERROR,
                                    "UANT: Failed to get Hk data, status = 0x%08X", status);
                ...
                break;
            case 2:
                status=ISIS_UANT_ReportAntennaActivationCount(2, &UANT_APP_Data.HkTlm.Payload.ant2actvcnt);
                if (status != CFE_SUCCESS)
                {
                    CFE_EVS_SendEvent(UANT_APP_GET_STATUS_ERR_EID, CFE_EVS_EventType_ERROR,
                                    "UANT: Failed to get Hk data, status = 0x%08X", status);
                ...
                break;
            case 3:
                status=ISIS_UANT_ReportAntennaActivationCount(3, &UANT_APP_Data.HkTlm.Payload.ant3actvcnt);
                if (status != CFE_SUCCESS)
                {
                    CFE_EVS_SendEvent(UANT_APP_GET_STATUS_ERR_EID, CFE_EVS_EventType_ERROR,
                                    "UANT: Failed to get Hk data, status = 0x%08X", status);
                ...
                break;
            case 4:
                status=ISIS_UANT_ReportAntennaActivationCount(4, &UANT_APP_Data.HkTlm.Payload.ant4actvcnt);
...
CFE_Status_t UANT_APP_NoopCmd(const UANT_APP_NoopCmd_t *Msg)
{
    ...
    CFE_EVS_SendEvent(UANT_APP_NOOP_INF_EID, CFE_EVS_EventType_INFORMATION, "UANT: NOOP command received.");
...
CFE_Status_t UANT_APP_Reset(const UANT_APP_ISIS_ResetCmd_t *Msg)
{
    ...
    report.CommandCode  = UANT_APP_RESET_CC;
...
CFE_Status_t UANT_APP_Arm(const UANT_APP_ISIS_ArmAntennaSystemsCmd_t *Msg)
{
    ...
    RPT_Report_t report = (RPT_Report_t){0};
    report.MsgID        = UANT_APP_CMD_MID;
    report.CommandCode  = UANT_APP_ARM_ANTENNA_SYSTEMS_CC;
    report.ReturnType   = (status == CFE_SUCCESS)
                          ? RPT_RETTYPE_SUCCESS
                          : RPT_RETTYPE_CFE;
    report.ReturnCode   = status;
...
CFE_Status_t UANT_APP_Disarm(const UANT_APP_ISIS_DisarmCmd_t *Msg)
{
    ...
    RPT_Report_t report = (RPT_Report_t){0};
    report.MsgID        = UANT_APP_CMD_MID;
    report.CommandCode  = UANT_APP_DISARM_CC;
    report.ReturnType   = (status == CFE_SUCCESS)
                          ? RPT_RETTYPE_SUCCESS
                          : RPT_RETTYPE_CFE;
    report.ReturnCode   = status;
...
CFE_Status_t UANT_APP_AutomatedDeployment(const UANT_APP_ISIS_AutomatedDeploymentCmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_AUTOMATED_DEPLOYMENT_CC;
...
CFE_Status_t UANT_APP_DeployAnt1(const UANT_APP_ISIS_DeployAnt1Cmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_DEPLOY_ANT1_CC;
        report.ReturnType    = (status == CFE_SUCCESS) ? RPT_RETTYPE_SUCCESS : RPT_RETTYPE_CFE;
        report.ReturnCode    = status;
        report.ReturnDataSize = 0;
...
CFE_Status_t UANT_APP_DeployAnt2(const UANT_APP_ISIS_DeployAnt2Cmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_DEPLOY_ANT2_CC;
        report.ReturnType    = (status == CFE_SUCCESS) ? RPT_RETTYPE_SUCCESS : RPT_RETTYPE_CFE;
        report.ReturnCode    = status;
        report.ReturnDataSize = 0;
...
CFE_Status_t UANT_APP_DeployAnt3(const UANT_APP_ISIS_DeployAnt3Cmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_DEPLOY_ANT3_CC;
        report.ReturnType    = (status == CFE_SUCCESS) ? RPT_RETTYPE_SUCCESS : RPT_RETTYPE_CFE;
        report.ReturnCode    = status;
        report.ReturnDataSize = 0;
...
CFE_Status_t UANT_APP_DeployAnt4(const UANT_APP_ISIS_DeployAnt4Cmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_DEPLOY_ANT4_CC;
        report.ReturnType    = (status == CFE_SUCCESS) ? RPT_RETTYPE_SUCCESS : RPT_RETTYPE_CFE;
        report.ReturnCode    = status;
        report.ReturnDataSize = 0;
...
CFE_Status_t UANT_APP_DeployAnt1_Override(const UANT_APP_ISIS_DeployAnt1OverrideCmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_DEPLOY_ANT1_OVERRIDE_CC;
        report.ReturnType    = (status == CFE_SUCCESS) ? RPT_RETTYPE_SUCCESS : RPT_RETTYPE_CFE;
        report.ReturnCode    = status;
        report.ReturnDataSize = 0;
...
CFE_Status_t UANT_APP_DeployAnt2_Override(const UANT_APP_ISIS_DeployAnt2OverrideCmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_DEPLOY_ANT2_OVERRIDE_CC;
        report.ReturnType    = (status == CFE_SUCCESS) ? RPT_RETTYPE_SUCCESS : RPT_RETTYPE_CFE;
        report.ReturnCode    = status;
        report.ReturnDataSize = 0;
...
CFE_Status_t UANT_APP_DeployAnt3_Override(const UANT_APP_ISIS_DeployAnt3OverrideCmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_DEPLOY_ANT3_OVERRIDE_CC;
        report.ReturnType    = (status == CFE_SUCCESS) ? RPT_RETTYPE_SUCCESS : RPT_RETTYPE_CFE;
        report.ReturnCode    = status;
        report.ReturnDataSize = 0;
...
CFE_Status_t UANT_APP_DeployAnt4_Override(const UANT_APP_ISIS_DeployAnt4OverrideCmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_DEPLOY_ANT4_OVERRIDE_CC;
        report.ReturnType    = (status == CFE_SUCCESS) ? RPT_RETTYPE_SUCCESS : RPT_RETTYPE_CFE;
        report.ReturnCode    = status;
        report.ReturnDataSize = 0;
...
CFE_Status_t UANT_APP_CancleDeployment(const UANT_APP_ISIS_CancelDeploymentActivationCmd_t *Msg)
{
    ...
    /* RPT 결과 보고 (데이터 없음) */
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID         = UANT_APP_CMD_MID;
        report.CommandCode   = UANT_APP_CANCEL_DEPLOYMENT_ACTIVATION_CC;
        report.ReturnType    = (status == CFE_SUCCESS) ? RPT_RETTYPE_SUCCESS : RPT_RETTYPE_CFE;
        report.ReturnCode    = status;
        report.ReturnDataSize = 0;
...
CFE_Status_t UANT_APP_GetDeploymentStatus(const UANT_APP_ISIS_ReportDeploymentStatusCmd_t *Msg)
{
    ...
    RPT_Report_t report = (RPT_Report_t){0};
    report.MsgID        = UANT_APP_CMD_MID;
    report.CommandCode  = UANT_APP_GET_DEPLOYMENT_STATUS_CC; /* CC 매크로명 맞춤 */
    report.ReturnType   = (status == CFE_SUCCESS)
                          ? RPT_RETTYPE_SUCCESS
                          : RPT_RETTYPE_CFE;
    report.ReturnCode   = status;
...
CFE_Status_t UANT_APP_MeasureAntSystemTemperature(const UANT_APP_ISIS_MeasureSystemTemperatureCmd_t *Msg)
{
    ...
    else
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID        = UANT_APP_CMD_MID;               
        report.CommandCode  = UANT_APP_MEASURE_SYSTEM_TEMPERATURE_CC; 
        report.ReturnType   = (status == CFE_SUCCESS)
                              ? RPT_RETTYPE_SUCCESS
                              : RPT_RETTYPE_CFE;                 
        report.ReturnCode   = status;
...
CFE_Status_t UANT_APP_ReportAntActivationCnt(const UANT_APP_ISIS_ReportAntActivationCntCmd_t *Msg)
{
    ...
    else
    {
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID        = UANT_APP_CMD_MID;
        report.CommandCode  = UANT_APP_REPORT_ANT_ACTIVATION_CNT_CC;
        report.ReturnType   = (status == CFE_SUCCESS)
                              ? RPT_RETTYPE_SUCCESS
                              : RPT_RETTYPE_CFE; 
        report.ReturnCode   = status;
...
CFE_Status_t UANT_APP_ReportAntActivationTime(const UANT_APP_ISIS_ReportAntActivationTimeCmd_t *Msg)
{
    ...
    if (status != CFE_SUCCESS) {
        ...
        RPT_Report_t report = (RPT_Report_t){0};
        report.MsgID        = UANT_APP_CMD_MID;
        report.CommandCode  = UANT_APP_REPORT_ANT_ACTIVATION_TIME_CC;
        report.ReturnType   = (status == CFE_SUCCESS)
                             ? RPT_RETTYPE_SUCCESS
                             : RPT_RETTYPE_CFE;
        report.ReturnCode   = status;
