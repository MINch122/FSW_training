...
void SAMPLE_APP_ProcessGroundCommand(const CFE_SB_Buffer_t *SBBufPtr)
{
    ...
    */
    switch (CommandCode)
    {
        ...
        case SAMPLE_APP_NATIVE_CAN_CC:
            if (SAMPLE_APP_VerifyCmdLength(&SBBufPtr->Msg, sizeof(SAMPLE_APP_NativeCANCmd_t)))
            {
                SAMPLE_APP_NativeCANCmd((const SAMPLE_APP_NativeCANCmd_t *)SBBufPtr);
            }
            break;
