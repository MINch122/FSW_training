...
CFE_Status_t PAYUEL_CAM_ShotCmd(const PAYUEL_CAM_ShotCmd_t *Msg)
{
    ...
    else
    {
        ...
        status = PAYUEL_CAM_ValidateResponse("Shot", PAYUEL_CAM_ID_SHOT, RxData, rx_len, sizeof(RxData),
                                             &return_type, NULL);
        if (status == CFE_SUCCESS && RxData[1] != 0x00U)
        {
            return_type = RPT_RETTYPE_HW;
            status   = PAYUEL_CAM_RejectHwStatus("Shot", RxData[1]);
        }
...
CFE_Status_t PAYUEL_CAM_HealthCheckCmd(const PAYUEL_CAM_HealthCheckCmd_t *Msg)
{
    ...
    else
    {
        ...
        status = PAYUEL_CAM_ValidateResponse("HealthCheck", PAYUEL_CAM_ID_HEALTH_CHECK, RxData, rx_len,
                                             sizeof(RxData), &return_type, NULL);
        if (status == CFE_SUCCESS && RxData[1] != 0x00U)
        {
            return_type = RPT_RETTYPE_HW;
            status   = PAYUEL_CAM_RejectHwStatus("HealthCheck", RxData[1]);
        }
        else if (status == CFE_SUCCESS)
        {
...
CFE_Status_t PAYUEL_CAM_ProcessBinningCmd(const PAYUEL_CAM_ProcessBinningCmd_t *Msg)
{
    ...
    else
    {
        ...
        status = PAYUEL_CAM_ValidateResponse("ProcessBinning", PAYUEL_CAM_ID_PROCESS_BINNING, RxData, rx_len,
                                             sizeof(RxData), &return_type, NULL);
        if (status == CFE_SUCCESS && RxData[1] != 0x00U)
        {
            return_type = RPT_RETTYPE_HW;
            status   = PAYUEL_CAM_RejectHwStatus("ProcessBinning", RxData[1]);
        }
        else if (status == CFE_SUCCESS)
        {
            PAYUEL_CAM_ClearImageMetaCache();
...
CFE_Status_t PAYUEL_CAM_DownloadMetaCmd(const PAYUEL_CAM_DownloadMetaCmd_t *Msg)
{
    ...
    {
        status = PAYUEL_CAM_RequestImageMeta(Msg->ImageSlot, Msg->ImageNumber, &Meta, &return_type, payload_error);
...
CFE_Status_t PAYUEL_CAM_SendBcnCmd(const PAYUEL_CAM_SendBcnCmd_t *Msg)
{
    ...
    rx_len = CFE_SRL_ApiTransactionCSP(PAYUEL_CAM_NODE, PAYUEL_CAM_PORT,
                                       TxData, sizeof(TxData), RxData, sizeof(RxData));
    PAYUEL_CAM_UnlockHardware("Beacon");

    status = PAYUEL_CAM_ValidateResponse("Beacon", PAYUEL_CAM_ID_BEACON, RxData, rx_len,
    ...
    payload = &RxData[1];
    PAYUEL_CAM_Data.bcn.Payload.PI_Boot_State     = payload[0];
    PAYUEL_CAM_Data.bcn.Payload.PI_Boot_Count     = PAYUEL_CAM_ReadU16BE(&payload[1]);
    PAYUEL_CAM_Data.bcn.Payload.CAM_Detect_State  = payload[3];
    PAYUEL_CAM_Data.bcn.Payload.CAM_Shutter_Count = PAYUEL_CAM_ReadU16BE(&payload[4]);
    PAYUEL_CAM_Data.bcn.Payload.CPU_Temp_x100     = PAYUEL_CAM_ReadI16BE(&payload[6]);
    PAYUEL_CAM_Data.bcn.Payload.Free_Disk_MB      = PAYUEL_CAM_ReadU32BE(&payload[8]);
...
CFE_Status_t PAYUEL_CAM_DownloadImageCmd(const PAYUEL_CAM_DownloadImageCmd_t *Msg)
{
    ...
    status = PAYUEL_CAM_QueueDownloadImage(Msg->ImageSlot, Msg->ImageNumber, &return_type);
    if (status != CFE_SUCCESS)
    {
